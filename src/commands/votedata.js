...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (verifyID(voteID, guildId) === false) {
            ...
            let data = JSON.parse(fs.readFileSync(`./data/vote/${guildId}/${voteID}.json`));
            ...
            await interaction.reply({
                embeds: [embed],
                files: [{ attachment: `./data/vote/${guildId}/${voteID}.json`, name: `votedata.json` }],
            });;
            return;
        }
    }
...
async function verifyID(voteID, guildId) {
    ...
    let file = fs.existsSync(`./data/vote/${guildId}/${voteID}.json`);
    if (file === false) {
        return false;
    }
    return true;
